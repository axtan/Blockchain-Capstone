...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(_owner == msg.sender, "You are not the contract owner");
        _;
    }

    modifier requireRealAddress(address account)
    {
        require(isRealAddress(account), "Not valid address");
        _;
    }


    function isRealAddress(address account) internal pure returns(bool)
    {
    ...
    function transferOwnership(address newOwner) public onlyOwner requireRealAddress(newOwner) {
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != ownerOf(tokenId),"Given address is already the owner of the token");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(_isApprovedOrOwner(from, tokenId), "Spender must be an approved owner to transfer.");

        
        // TODO: require token is being transfered to valid address
        require(isRealAddress(to), "Receiver must be a valid address.");
...
contract ERC721Mintable is ERC721Metadata('agallegos', 'AGT', 'https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/')
{
    constructor() public {
